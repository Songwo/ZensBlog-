...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!project) return errorJson("项目不存在", 404);
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rate = await checkRateLimit(request, { namespace: "api-project-update", limit: 60, windowMs: 60_000 });
  ...
  try {
    ...
    if (!existing) return errorJson("项目不存在", 404);
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rate = await checkRateLimit(request, { namespace: "api-project-delete", limit: 30, windowMs: 60_000 });
  if (!rate.allowed) return errorJson("请求过于频繁，请稍后再试", 429);
